...
**Versión:** 2.3 — corregidas 5 rutas de perfil que estaban en español y no existían (`quotes`/`messages`/`verification`/`settings`/`services` en vez de `cotizaciones`/`mensajes`/`verificacion`/`ajustes`/tab), quitadas las referencias a Sponsors (módulo eliminado) y corregido el mecanismo real de scope de Admin por país
...
| Mis servicios | `/{country}/profile/services` |
| Leads (mini-CRM) | `/{country}/profile/leads` |
| Cotizaciones | `/{country}/profile/quotes` |
| Mensajes | `/{country}/profile/messages` |
| Verificación de identidad (KYC) | `/{country}/profile/verification` |
| Ajustes de cuenta | `/{country}/profile/settings` |
...
| Favoritos | `/{country}/profile/favorites` |
| Mensajes | `/{country}/profile/messages` |
| Cotizaciones recibidas | `/{country}/profile/quotes` |
| Ajustes de cuenta | `/{country}/profile/settings` |
...
- **Scope de Admin por país (corregido 2026-08-24):** no existe un campo `adminCountries` en el JWT. El scope real es `UserRole.countryId` (relación a `Country`, nullable). El payload del JWT lleva un único campo `country` (no un array), calculado en `computeCountry()` (`auth.service.ts`) como el código del primer `UserRole` que tenga país asignado — si un usuario tuviera roles en más de un país, el JWT solo refleja uno. `RolesGuard` (`roles.guard.ts`) valida el rol pero **no filtra por país en absoluto**; sigue siendo responsabilidad de cada service validar el país.
