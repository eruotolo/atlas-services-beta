...
> ⚠️ **Sin re-verificar contra código desde su generación (más de 2 meses).** Solo se corrigió la mención a "sponsors" (módulo eliminado del backend). El resto de las ~250 filas no fue re-auditado contra `appmobile/src` en esta pasada — antes de usarlo para priorizar trabajo de mobile, conviene regenerarlo.
